...
public class ResponseConverter {
    ...
    public static ArrayList<MovieModel> movieResponseConvert(MoviesListResponse body) {
        ...
        for(MovieResponse movieResponse : body.getResults()) {
            ...
            MovieModel currentMovie =
                    new MovieModel(movieResponse.getId(), movieResponse.getTitle(), movieResponse.getOverview(),
    ...
    public static String getTrailerUrl(VideosListResponse body) {
            List<VideoResponse> results = body.getResults();
            if (results != null && !results.isEmpty()) {
                return MoviesService.YOUTUBE_URL + results.get(0).getKey();
            }
        return null;
    }
}
